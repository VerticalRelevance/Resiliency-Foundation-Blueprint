...
class ResiliencyFoundationLambdaStack(Stack):
    ...
    def uploadLambdaCode(self, random_bucket_suffix, domain_name, owner, repo_name):
        ...
        print(
            "aws codeartifact login --tool pip --domain {} --domain-owner {} --repository {}".format(
                domain_name, owner, repo_name
            )
        )
        os.system(
            "aws codeartifact login --tool pip --domain {} --domain-owner {} --repository {}".format(
        ...
        os.system(
            "pip install -r {} -t ../resiliency_code/lambda".format(
                resiliency_reqs_path
            )
        )
        ...
        lambda_code_bucket = s3.Bucket(
            self,
            "resiliency_lambda_code_bucket" + random_bucket_suffix,
    ...
    def uploadExperiments(self, random_bucket_suffix):
        ...
        experiment_bucket = s3.Bucket(
            self,
            "resiliency-experiment-bucket-" + random_bucket_suffix,
            bucket_name="resiliency-experiment-bucket" + random_bucket_suffix,
            access_control=s3.BucketAccessControl.PRIVATE,
        )
    ...
    def __init__(self, scope, id):
        ...
        s3_key = ResiliencyFoundationLambdaStack.createKMSKey(
            self, "s3_key", "Customer managed KMS key to encrypt S3 resources"
        )
        resiliency_lambda_role = ResiliencyFoundationLambdaStack.createIAMRole(
        ...
        code_upload_resources = ResiliencyFoundationLambdaStack.uploadLambdaCode(
            self, random_bucket_suffix, domain_name, owner, repo_name
        )
        code_upload = code_upload_resources["code_upload"]
        lambda_code_bucket = code_upload_resources["lambda_code_bucket"]
        experiment_resources = ResiliencyFoundationLambdaStack.uploadExperiments(
            self, random_bucket_suffix
        )

        lambda_function = ResiliencyFoundationLambdaStack.createFunction(
            self, resiliency_lambda_role, lambda_code_bucket, code_upload
        )
